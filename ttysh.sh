...
printf "\n%s\n" "Your /home/"$USER"/.bookmarks_ttysh.html is now formatted for the (b) command"
}
...
while [ 1 ]; do
	...
	printf "\n%s\n%s\n%s\n%s\n\n"	"sm to enter creator and title" "m to download music url from xclip/wl-clipboard" "ytm to run again" "q to quit"
...
printf "\n%s" ""
	...
	case "$answer" in
		...
		screenshotcheck
		sudo fbgrab -c 6 screenshot"$timestamp".png
		;;
